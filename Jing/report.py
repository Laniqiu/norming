...
def main(model_path, fdir, svec_dir, out_dir, dim=768):
    ...
    for sd_folder in sorted(fdir.iterdir()):
        if not sd_folder.is_dir():
            continue
        for folder in sorted(sd_folder.iterdir()):
            if not folder.is_dir():
                continue
            tgt = folder.name.strip()  # target word
            # todo 需要删除的代码
            if tgt not in tgt_folder:  # sense emb文件夹
                continue
            ...
            #     if year.name.startswith("."):  # skip 隐藏文件
            #         continue
            #     process_one_year(year, tgt, this_outdr, model, tokenizer, mat, ss)
            # print("用时", time() - st)
            # breakpoint()
